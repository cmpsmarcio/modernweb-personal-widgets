      ...
      <article className="border-l-4 border-blue-400 bg-blue-100 sm:grid grid-cols-5 bg-white shadow-lg p-7 relative lg:mx-auto sm:p-4 rounded-lg lg:col-span-2 lg:ml-20" >
        ...
        <div className="pt-5 self-center sm:pt-0 sm:pl-10 col-span-3">
          ...
          <div className="float-none px-2 pt-5 pb-3">  
            ...
            <label className="text-xs">dólar: R${parseFloat(price.USD.low).toFixed(2)} - R${parseFloat(price.USD.high).toFixed(2)}</label>
          </div>
          <div className="float-none px-2 pb-5">
            <label className="text-xs">euro: R${parseFloat(price.EUR.low).toFixed(2)} - R${parseFloat(price.EUR.high).toFixed(2)}</label>
        ...
        <div className="justify-self-end">
          <img src="https://cdn4.iconfinder.com/data/icons/success-filloutline/64/save-money-coin-piggy_bank-deposit-128.png" alt="Bookmark" className="w-4 sm:w-8 absolute top-3 right-3 sm:relative sm:top-0 sm:right-0" />
        </div>
      </article>
    </div>
  )
